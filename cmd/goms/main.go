...
func main() {
	...
	case "make:controller":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:controller <ControllerName>")
			os.Exit(1)
		}
	...
	case "make:model":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:model <ModelName>")
			os.Exit(1)
		}
	...
	case "make:migration":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:migration <migration_name>")
			os.Exit(1)
		}
	...
	case "make:service":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:service <ServiceName>")
			os.Exit(1)
		}
	...
	case "make:request":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:request <RequestName>")
			os.Exit(1)
		}
	...
	case "make:middleware":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:middleware <MiddlewareName>")
			os.Exit(1)
		}
	...
	case "make:seeder":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:seeder <SeederName>")
			os.Exit(1)
		}
	...
	case "make:resource":
		if len(os.Args) < 3 {
			...
			fmt.Println("Usage: goms make:resource <ResourceName>")
			os.Exit(1)
		}
	...
	default:
		...
		fmt.Println("Run 'goms list' to see available commands")
		os.Exit(1)
	}
}
...
func printUsage() {
 ...
 | |_| | (_) | |  | |  __/ | | |
  ...
  GoMen CLI - Code Generator

Usage:
  goms <command> [arguments]
...
Examples:
  goms make:controller Product
  goms make:model Product
  goms make:migration create_products_table
  goms make:resource Product
`)
}
